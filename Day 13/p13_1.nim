...
type
  ...
  Crash = object              # Description of a possible crash.
    ...
    of false: discard         # No crash.
    of true: pos: Position    # Position of the crash.
...
proc moveCart(tracks: Tracks, cart: var Cart): Crash =
  ...
  for othercart in carts:
    if othercart.num != cart.num and othercart.pos == cart.pos:
      return Crash(hasCrashed: true, pos: cart.pos)
...
# Process a tick.
proc doTick(tracks: Tracks, carts: var seq[Cart]): Crash =
...
while true:
  let crash = tracks.doTick(carts)
  if crash.hasCrashed:
    echo crash.pos.x, ',', crash.pos.y
    break
